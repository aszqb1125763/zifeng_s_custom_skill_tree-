...
public class Config {
    ...
    public static final ModConfigSpec.BooleanValue HEALTH_BAR_COMPRESS;
    /** 是否在血条左侧显示真实血量数字 */
    public static final ModConfigSpec.BooleanValue HEALTH_NUMBER_ENABLED;
    /** 属性面板数据源：true=直接读取原版实时属性值（含装备/药水/其他模组加成），false=旧行为（仅技能内部计算） */
    public static final ModConfigSpec.BooleanValue PANEL_USE_VANILLA_ATTR;
    ...
    /** Aura Damage: damage multiplier per level (0.10 = +10%/lvl, multiplicative). 光环伤害倍率 */
    ...
    public static final ModConfigSpec.DoubleValue BODY_HP_PER_POINT;
    /** Body: +X armor per point (default 0.2). 体魄护甲每点 */
    public static final ModConfigSpec.DoubleValue BODY_ARMOR_PER_POINT;
    ...
    public static final ModConfigSpec.DoubleValue BLADE_DAMAGE_PER_POINT;
    /** Attack Speed: +X attack speed per point (default 0.02). 疾攻攻速每点 */
    public static final ModConfigSpec.DoubleValue ATTACK_SPEED_PER_POINT;
    /** Mining: +X mining speed per point (default 0.3). 采掘挖速每点 */
    public static final ModConfigSpec.DoubleValue MINING_SPEED_PER_POINT;
    /** Move: +X movement speed per point (default 0.005). 疾行移速每点 */
    public static final ModConfigSpec.DoubleValue MOVE_SPEED_PER_POINT;
    /** Luck: +X luck per point (default 0.1). 幸运每点 */
    public static final ModConfigSpec.DoubleValue LUCK_PER_POINT;
    /** Jump: +X jump strength per point (default 0.01). 跳跃每点 */
    public static final ModConfigSpec.DoubleValue JUMP_PER_POINT;
    /** Fly: +X fly speed per point (default 0.005). 飞行每点 */
    public static final ModConfigSpec.DoubleValue FLY_SPEED_PER_POINT;
    /** Swim: +X swim speed per point (default 0.005). 游泳每点 */
    public static final ModConfigSpec.DoubleValue SWIM_SPEED_PER_POINT;
    /** Armor Truth: +X damage reduction per point (default 0.005 = 0.5%). 金身减伤每点 */
    public static final ModConfigSpec.DoubleValue AMP_ARMOR_DR_PER_POINT;
    /** Reach: +X blocks reach & attack range per level (default 1.0). 长臂距离每级 */
    public static final ModConfigSpec.DoubleValue REACH_PER_LEVEL;
    /** KB Resist: +X knockback resistance per level (default 0.1 = 10%). 稳如泰山每级 */
    public static final ModConfigSpec.DoubleValue KB_RESIST_PER_LEVEL;
    // Amplify per-point percentages (11; attack speed / mining speed differ, rest 0.1)
    // 增幅列每点百分比（11 项；攻速/挖速独立，其余 0.1）
    /** Amp HP: +X max health multiplier per point (default 0.1 = +10%). 血魄真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_HP_PER_POINT;
    /** Amp Tough: +X toughness/KB multiplier per point (default 0.1 = +10%). 磐石真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_TOUGH_PER_POINT;
    /** Amp Luck: +X luck multiplier per point (default 0.1 = +10%). 鸿运真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_LUCK_PER_POINT;
    /** Amp Damage: +X damage multiplier per point (default 0.1 = +10%). 剑心真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_DAMAGE_PER_POINT;
    /** Amp Attack Speed: +X attack speed multiplier per point (default 0.08 = +8%). 疾风真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_ATTACK_SPEED_PER_POINT;
    /** Amp Mining: +X mining speed multiplier per point (default 0.12 = +12%). 破岩真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_MINING_PER_POINT;
    /** Amp Move: +X movement multiplier per point (default 0.1 = +10%). 健步真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_MOVE_PER_POINT;
    /** Amp Jump: +X jump multiplier per point (default 0.1 = +10%). 蹦跳真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_JUMP_PER_POINT;
    /** Amp Fly: +X fly speed multiplier per point (default 0.1 = +10%). 御空真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_FLY_PER_POINT;
    /** Amp Swim: +X swim multiplier per point (default 0.1 = +10%). 游鱼真解每点 */
    public static final ModConfigSpec.DoubleValue AMP_SWIM_PER_POINT;
    ...
    static {
        ...
        builder.comment("Per-point attribute bonuses (added 2026-08-29): all per-point values for base / multi-level ultimate / amplify skills. Hot-reload applies.\n属性每点加成")
