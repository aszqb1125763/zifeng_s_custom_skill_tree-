...
public class ModKeyBindingEvents {
    ...
    private static final Map<String, org.zifeng.skilltree.data.OperZone> operZonesClient = new HashMap<>();
     ...
     * 底层存结构化 "dim|name|x|y|z"（2026-09-08），由 setLootVacuumBindClient 解析出维度/坐标。
     */
    ...
    public static void onClientTick(ClientTickEvent.Pre event) {
        ...
        if (Minecraft.getInstance().screen == null) { // 界面打开时不触发（避免与设置窗口/技能树交互冲突）
            ...
            // ⚠️ 2026-09-07 规范 v1.0：模式循环统一读 Skills 注册表（getModeCount 态数）；
            //    按模式类型分派（敌我目标/搬运 → AuraTarget；天气 → WeatherMode）；等级循环保持。
        ...
        // ⚠️ 2026-09-07 规范 v1.0：是否允许当前屏幕触发由 Skills.canTriggerInScreen 决定
        //（搬运术 = 容器 GUI 也可触发；闪现 = 正常游戏内触发）。
        // 串联逻辑：子1级（开关键）未激活（技能关闭）→ 子3级（触发键）无功能。
            ...
            for (String skillId : org.zifeng.skilltree.client.SkillKeyBinds.allTriggerBinds().keySet()) {
                if (org.zifeng.skilltree.client.SkillKeyBinds.consumeTriggerClick(skillId)) {
                    ...
                    if (Skills.isContainerHaul(skillId)) {
                        ...
                        org.zifeng.skilltree.network.ModNetwork.sendToServer(new org.zifeng.skilltree.network.ZoneC2SPacket(10, "",
                                0, 0, 0, 0, 0, 0));
                    } else if (Skills.MACHINE_ZONE_EXCAVATE.equals(skillId)) {
                        // 机械共鸣·选区挖掘：按触发键瞬间挖空全区（2026-09-08）
                        org.zifeng.skilltree.network.ModNetwork.sendToServer(new org.zifeng.skilltree.network.ZoneC2SPacket(11, "",
                                0, 0, 0, 0, 0, 0));
                    }
                }
            }
