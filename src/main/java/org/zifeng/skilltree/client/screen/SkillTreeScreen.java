...
import org.zifeng.skilltree.network.WeatherModeC2SPacket;
...
public class SkillTreeScreen extends Screen {
    ...
    public void tick() {
        ...
        if (tickCounter >= 40) {
            tickCounter = 0;
            org.zifeng.skilltree.network.ModNetwork.sendToServer(new OpenSkillTreeC2SPacket(true));
        }
    }

    private int tickCounter = 0;

    /** 十纵列布局：十列顶部对齐（上方对齐），魔法增幅列在最左，工具列在最右（2026-09-08 新增第10列）。 */
    private void rebuildButtons() {
        buttons.clear();
        // 10 列中心 x：间距统一 300（按钮+3键框不重叠）
    ...
    private void renderLayer4SkillTree(GuiGraphics guiGraphics) {
        ...
        for (int i = 0; i < colCenters.length; i++) {
            ...
            if (hasLevelBindable) {
                ...
                // 2026-09-07：光环列第二键标题从"目标"改为"模式"（光环敌我目标/搬运术自动手动统一叫模式，更宽泛）
        ...
        if (Skills.isStickTool(skillId)) {
            ...
            for (int m = 0; m < Skills.stickModeCount(); m++) {
                if (m == Skills.STICK_MODE_BIND) { // 绑定模块=两绑技能
                    boolean unlocked = bindSkill;
                    String label = "  " + (unlocked ? "✓ " : "✗ ") + Component.translatable(
                    ...
                    lines.add(new TooltipLine(label, unlocked ? 0xFF55FF55 : 0xFF777777, 1.0F));
                    continue;
                }
    ...
    /**
     * 属性值读取（数据源可切换，2026-09-11；2026-09-11 二次：面板标题栏新增可直接点的开关）：
     * <ul>
     ...
     * </ul>
     * 取值优先级：属性面板标题栏按钮（本地点过的）&gt; 配置项 {@code panelUseVanillaAttributes}。
     * 客户端属性值由 {@code ClientboundUpdateAttributesPacket} 同步，因此能读到服务端技能加成。
     * 若某个属性未被同步，读到的会是基础值 → 可点面板按钮切回旧行为。
     */
                ...
                if (skillButton.isHovered(mouseX, mouseY, this)) {
                    ...
                    if (isShiftHeld() && Skills.hasModeCycle(skillId)) {
                        if (Skills.AURA_WEATHER.equals(skillId)) {
                            ...
                            org.zifeng.skilltree.network.ModNetwork.sendToServer(new WeatherModeC2SPacket(next));
                        } else {
    ...
    public void onClose() {
        ...
        org.zifeng.skilltree.network.ModNetwork.sendToServer(new OpenSkillTreeC2SPacket(false));
        super.onClose();
    }
