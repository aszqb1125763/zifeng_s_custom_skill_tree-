...
public class SkillPointConverterScreen extends AbstractContainerScreen<SkillPointConverterMenu> {
    ...
    public static void registerScreens(net.minecraftforge.fml.event.lifecycle.FMLClientSetupEvent event) {
        // ⚠️ 1.20.1：MenuScreens.register 必须在 FMLClientSetupEvent（注册表冻结后）调用，
        //    构造阶段调用会 RegistryObject.get() 崩溃（注册表未填充）
        net.minecraft.client.gui.screens.MenuScreens.register(ModMenus.SKILL_POINT_CONVERTER.get(), SkillPointConverterScreen::new);
    }
}
