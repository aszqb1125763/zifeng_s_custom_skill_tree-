 ...
 *
 * <h3>为什么需要它</h3>
 * 本模组的 4 个世界渲染器都需要判断「当前是否处于 Iris 阴影 pass」以便跳过绘制
 * （避免轮廓线在阴影里闪烁）。为避免编译期依赖 Iris jar，用反射调用
 * {@code net.irisshaders.iris.api.v0.IrisApi}。
 *
 * <h3>⚠️ 修复的问题（原实现每帧抛异常）</h3>
 * 原代码把 {@code isIrisShadowPass()} 复制到 4 个渲染器里，且在
 * {@code RenderLevelStageEvent} 回调（每帧）的<b>最前面</b>调用。未装 Iris 时：
 * <pre>
 *   IRIS_GET_INSTANCE == null  →  Class.forName(...)  每次都抛 ClassNotFoundException
 *   （异常发生在赋值前 → 变量永远为 null → 没有任何"已失败"缓存）
 * </pre>
 * 结果是 <b>每帧抛出 4 个异常</b>（含 fillInStackTrace 填充调用栈）→ 未装 Iris 的
 * 整合包下约 <b>240 次异常构造/秒</b>。
 *
 * <h3>修复方案</h3>
 * <ul>
 *   <li>统一到本类（消除 4 份重复代码）</li>
 *   <li>用 {@code resolveAttempted} 标记记录「<b>已尝试解析</b>」：
 *       即使失败也只 {@code Class.forName} 一次，之后直接短路返回，<b>零异常</b></li>
 *   <li>{@code Method} 对象成功后缓存（与原实现一致）</li>
 * </ul>
 * 装了 Iris 时行为完全不变（仍每帧 3 次反射调用，开销可忽略）；
 * 没装 Iris 时从「每帧抛异常」变为「首次解析失败后永久短路」。
 */
