...
public class UltimateEvents {
     ...
     * 匹配规则（2026-08-28 通用化，支持【用产物 ID 加黑名单】）：
     *  1. 精确匹配：黑名单项 == 掉落物（如添加 coal_ore 拦 coal_ore）
     *  2a. 黑名单项 == 掉落物的熔炼产物：黑名单=铁锭，掉落=任何模组铁矿石/粗铁（熔炼出铁锭）→ 拦截；
     *      黑名单=木炭，掉落=任何模组原木（熔炼出木炭）→ 拦截。最通用，不同模组矿石/原木统一被产物拦住。
     *  2b. 黑名单矿石与掉落物熔炼出【同一种产物】→ 视为同类矿石（黑名单=gold_ore，掉落=raw_gold → 都出 gold_ingot）。
     */
    public static boolean isAutoSmeltBlacklisted(PlayerSkillRecord record, Item item,
        ...
        // 2. 熔炼产物关联（双向，2026-08-28 同步 1.20.1 修复 + 通用化）：
     ...
     *   <li>装备栏来源 + 持久生物 → 仅放行神化词条装（带 FROM_* 来源标记的世界产出）</li>
     * </ul>
     */
    private static boolean isPlayerInjectedEquipment(java.util.List<ItemStack> equippedSnapshot,
        ...
        // 持久怪（词条 Boss 命名/召唤方块/捡过玩家物品）：仅世界产出词条装豁免
